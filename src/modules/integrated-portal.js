...
function enhanceRegistration() {
  ...
  const channel = field(
    "Forma de pagamento",
    '<select name="paymentChannel"></select>',
  );
  ...
  function renderPaymentArea() {
    ...
    cardDetails
      .querySelector("[data-open-card-form]")
  ...
  plan
    ...
    .addEventListener("change", updateContractOptions);
  billingCycle
    .querySelector("select")
    .addEventListener("change", renderPaymentArea);
  const requestedPlan = new URLSearchParams(
    location.hash.split("?")[1] || "",
  ).get("plan");
  if (
    [...plan.querySelector("select").options].some(
...
function prescriptionValues(form) {
  return [...form.querySelectorAll("[data-workout-prescription]")].map(
    (row) => ({
      exerciseId: row.dataset.exerciseId,
      sessionLabel:
        row.querySelector('[name="prescriptionSession"]')?.value || "A",
      sets: row.querySelector('[name="prescriptionSets"]')?.value || "3",
      repetitions:
        row.querySelector('[name="prescriptionRepetitions"]')?.value || "10-12",
      restSeconds:
        row.querySelector('[name="prescriptionRestSeconds"]')?.value || "60",
      notes: row.querySelector('[name="prescriptionNotes"]')?.value || "",
    }),
  );
}

function renderWorkoutPrescriptionBuilder(
  form,
  saved = prescriptionValues(form),
) {
  const builder = form.querySelector("[data-workout-prescriptions]");
  const select = form.elements.exerciseIds;
  if (!builder || !select) return;
  const values = new Map(saved.map((item) => [String(item.exerciseId), item]));
  ...
  if (!selected.length) {
    ...
    empty.textContent =
      "Selecione os exercícios acima para definir cada prescrição.";
  ...
  selected.forEach((option, index) => {
    ...
    row.dataset.exerciseId = option.value;
    row.innerHTML = `<header><span></span><strong></strong></header><div class="field-grid field-grid--four"><label class="field"><span>Treino</span><select name="prescriptionSession"><option>A</option><option>B</option><option>C</option><option>D</option><option>E</option><option>F</option></select></label><label class="field"><span>Séries</span><input name="prescriptionSets" type="number" min="1" max="20" required></label><label class="field"><span>Repetições ou tempo</span><input name="prescriptionRepetitions" maxlength="40" required></label><label class="field"><span>Intervalo (segundos)</span><input name="prescriptionRestSeconds" type="number" min="0" max="1800" required></label></div><label class="field"><span>Observação individual</span><input name="prescriptionNotes" maxlength="500" placeholder="Ex.: cadência controlada, última série até a falha"></label>`;
    row.querySelector("header span").textContent = String(index + 1).padStart(
      2,
      "0",
    );
    row.querySelector("header strong").textContent =
      `${exercise?.name || option.textContent} · ${exercise?.group || "Grupo muscular"}`;
    row.querySelector('[name="prescriptionSession"]').value =
      current.sessionLabel || "A";
    row.querySelector('[name="prescriptionSets"]').value = current.sets || 3;
    row.querySelector('[name="prescriptionRepetitions"]').value =
      current.repetitions || "10-12";
    row.querySelector('[name="prescriptionRestSeconds"]').value =
      current.restSeconds ?? 60;
...
function enhanceWorkout() {
  ...
  help.textContent =
    "Selecione um ou vários exercícios. Depois personalize cada prescrição.";
  select.append(help);
  const prescription = document.createElement("div");
  prescription.className = "workout-prescription-builder";
  prescription.dataset.workoutPrescriptions = "";
  ...
  select
    ...
    .addEventListener("change", () =>
      renderWorkoutPrescriptionBuilder(select.closest("form")),
    );
  renderWorkoutPrescriptionBuilder(body.closest("form"));
}
...
function enhanceExercise() {
  ...
  const picker = field(
    ...
    "<select data-video-picker></select>",
  );
  ...
  muscleGroups.forEach((group) => {
    const items = exerciseVideoLibrary.filter(
  ...
  pickerSelect.addEventListener("change", () =>
    applyCatalogItem(
      mediaExerciseCatalog.find((entry) => entry.id === pickerSelect.value),
    ),
  );
  name.addEventListener("change", () => {
    applyCatalogItem(
      mediaExerciseCatalog.find((entry) => entry.name === name.value),
    );
  });
}
...
function readyPrescriptionValues(form) {
  return [...form.querySelectorAll("[data-ready-prescription]")].map((row) => ({
    exerciseId: row.dataset.exerciseId,
    sessionLabel: row.querySelector('[name="sessionLabel"]').value,
...
function renderReadyPrescriptionBuilder(
  form,
  saved = readyPrescriptionValues(form),
) {
  ...
  [...select.selectedOptions].forEach((option, index) => {
    ...
    row.dataset.exerciseId = option.value;
    row.innerHTML = `<header><span>${String(index + 1).padStart(2, "0")}</span><strong></strong></header><div class="field-grid field-grid--four"><label class="field"><span>Treino</span><select name="sessionLabel"><option>A</option><option>B</option><option>C</option><option>D</option><option>E</option><option>F</option></select></label><label class="field"><span>Séries</span><input name="sets" type="number" min="1" max="20" value="3" required></label><label class="field"><span>Repetições/tempo</span><input name="repetitions" value="10-12" required></label><label class="field"><span>Intervalo (s)</span><input name="restSeconds" type="number" min="0" max="1800" value="60" required></label></div><label class="field"><span>Observação</span><input name="notes" maxlength="500" placeholder="Ex.: cadência controlada"></label>`;
    row.querySelector("strong").textContent =
      `${exercise?.name || option.textContent} · ${exercise?.group || "Grupo muscular"}`;
    row.querySelector('[name="sessionLabel"]').value =
      current.sessionLabel || "A";
    row.querySelector('[name="sets"]').value = current.sets || 3;
    row.querySelector('[name="repetitions"]').value =
      current.repetitions || "10-12";
  ...
  if (!builder.children.length) {
    ...
    empty.className = "workout-builder-empty";
    empty.textContent =
      "Selecione exercícios para montar os treinos A, B, C, D e E.";
    builder.append(empty);
  }
}
...
function openReadyProgramDialog(program = null) {
  ...
  form.elements.duration.value = program?.duration || "12 semanas";
  ...
  try {
    prescriptions =
      ...
      JSON.parse(program?.exercisePrescriptionsJson || "[]");
  } catch {
    prescriptions = [];
  }
  const selected = new Set(
    prescriptions.map((item) => String(item.exerciseId)),
  );
  form.elements.exerciseIds.replaceChildren(
    ...getData().exercises.map((exercise) => {
      const option = document.createElement("option");
      option.value = exercise.id;
      option.textContent = `${exercise.name} — ${exercise.group}`;
      option.selected = selected.has(String(exercise.id));
      return option;
    }),
  );
  renderReadyPrescriptionBuilder(form, prescriptions);
  dialog.showModal();
}
...
function createReadyWorkoutLibraryPanel() {
  ...
  panel.innerHTML = `<div class="panel-heading"><div><span class="eyebrow eyebrow--blue">Produto de valor único</span><h2>Treinos Prontos — acesso permanente</h2><p>Monte o PDF completo dentro do sistema. Ao publicar, ele aparece automaticamente para todos os compradores desta modalidade.</p></div><button class="button button--primary" type="button" data-new-ready-program>+ Novo treino pronto</button></div><div class="media-library-grid" data-ready-workout-grid></div>`;
  page.append(panel);

  const dialog = document.createElement("dialog");
  dialog.className = "modal modal--wide";
  dialog.dataset.readyProgramDialog = "";
  dialog.innerHTML = `<form method="dialog"><header><div><span class="eyebrow eyebrow--blue">Ficha geral ABCDE</span><h2>Montar treino pronto completo</h2></div><button class="icon-button" type="button" data-ready-close aria-label="Fechar">×</button></header><div class="modal-body"><label class="field"><span>Nome do programa</span><input name="name" required placeholder="Ex.: Masculino avançado ABCDE"></label><div class="field-grid field-grid--three"><label class="field"><span>Objetivo</span><select name="goal"><option>Hipertrofia</option><option>Emagrecimento</option><option>Condicionamento</option><option>Força</option></select></label><label class="field"><span>Nível</span><select name="level"><option>Iniciante</option><option selected>Intermediário</option><option>Avançado</option></select></label><label class="field"><span>Duração</span><select name="duration"><option>4 semanas</option><option>8 semanas</option><option selected>12 semanas</option><option>16 semanas</option></select></label></div><div class="field-grid"><label class="field"><span>Cor da ficha</span><select name="colorTheme"><option value="red">Vermelho e grafite</option><option value="blue">Azul FRS</option><option value="green">Verde</option><option value="black">Preto</option></select></label><label class="field"><span>Descrição</span><input name="description" placeholder="Resumo e orientações gerais"></label></div><label class="field"><span>Exercícios do programa completo</span><select name="exerciseIds" multiple size="8" required></select><small>Use Ctrl/Cmd para selecionar vários. Em seguida, indique A, B, C, D ou E em cada exercício.</small></label><div class="workout-prescription-builder" data-ready-prescriptions></div><label class="check-field"><input name="published" type="checkbox" value="1"><span>Publicar para todos que compraram Treinos Prontos</span></label><p role="status" aria-live="polite"></p></div><footer><button class="button button--secondary" type="button" data-ready-close>Cancelar</button><button class="button button--primary" type="submit">Salvar treino pronto</button></footer></form>`;
  document.body.append(dialog);
  const form = dialog.querySelector("form");
  form.elements.exerciseIds.addEventListener("change", () =>
...
function renderReadyWorkoutLibrary() {
  ...
  programs.forEach((program) => {
    try {
      program.exercisePrescriptions = JSON.parse(
        program.exercisePrescriptionsJson || "[]",
      );
    } catch {
      program.exercisePrescriptions = [];
    }
    const sessions = [
      ...new Set(
        program.exercisePrescriptions.map((item) => item.sessionLabel),
      ),
    ].sort();
...
function confirmExerciseVideoDeletion(exercise) {
  ...
  dialog.querySelector("[data-exercise-video-name]").textContent =
    exercise.name;
  ...
  videosByMuscleGroup(videos).forEach((group) => {
    ...
    group.exercises.forEach((exercise) => {
      ...
      instructions.textContent =
        exercise.instructions || "Sem instruções adicionais.";
...
function createAccessDialog() {
  ...
  window.addEventListener("frs:manage-access", (event) => {
    ...
    form.querySelector("[data-access-student]").textContent =
      `${student.name} · ${student.email}`;
    form.elements.planCode.value = student.planCode || "basic";
    form.elements.billingCycle.value = [
      "monthly",
      "quarterly",
      "semiannual",
      "annual",
    ].includes(student.billingCycle)
...
function connectPlanCards() {
  ...
  document.querySelectorAll(".plan-card").forEach((card, index) => {
    ...
    link.textContent =
      index === 0 ? "Escolher Treinos Prontos" : "Escolher este plano";
...
function renderOperations() {
  ...
  if (!pending.length && !checkins.length) {
    content.append(
      document.createTextNode("Nenhuma solicitação ou check-in pendente."),
    );
    return;
  }
  pending.slice(0, 6).forEach((student) => {
    ...
    release.addEventListener("click", async () => {
      ...
      try {
        ...
        showToast(
          "Pagamento confirmado. O conteúdo já está liberado para o aluno.",
        );
        window.dispatchEvent(new Event("frs:remote-refresh"));
      } catch (error) {
        showToast(error.message);
    ...
    button.addEventListener("click", () =>
      window.dispatchEvent(
        new CustomEvent("frs:manage-access", { detail: student.id }),
      ),
    );
    item.append(
...
export function initIntegratedPortal() {
  ...
  window.addEventListener("frs:edit-workout", (event) => {
    ...
    if (!Array.isArray(workout.exercisePrescriptions))
      try {
        workout.exercisePrescriptions = JSON.parse(
    ...
    queueMicrotask(() => {
      ...
      renderWorkoutPrescriptionBuilder(form, workout.exercisePrescriptions);
    });
  });
