...
function renderStudents() {
  ...
  table.replaceChildren(
    ...filtered.map((student) => {
      ...
      row.querySelector('[data-cell="date"]').textContent = formatDate(
...
function renderRecentStudents() {
  const rows = getData()
    ...
    .map((s) => {
      ...
      row.querySelector(".person-cell small").textContent =
        `${s.email} · ${planSummary(s)}`;
      row.querySelector('[data-cell="goal"]').textContent = s.goal;
      row.querySelector('[data-cell="workout"]').textContent =
        s.workout || "Aguardando ficha";
      row.querySelector('[data-cell="activity"]').textContent =
        s.activity || "Novo cadastro";
      const status = row.querySelector(".status");
      status.textContent = accessLabel(s);
      status.classList.add(
        s.accessStatus === "active" ? "status--active" : "status--paused",
      );
      const view = row.querySelector("button");
      view.setAttribute("aria-label", `Editar ${s.name}`);
      view.addEventListener("click", () =>
        window.dispatchEvent(
          new CustomEvent("frs:edit-student", { detail: String(s.id) }),
        ),
      );
      return row;
    });
...
function renderWorkouts() {
  ...
  document.querySelector("[data-workouts-grid]").replaceChildren(
    ...workouts.map((w) => {
      ...
      card.querySelector(".workout-progress strong").textContent =
        `${w.progress}%`;
...
function renderAssessments() {
  document.querySelector("[data-assessments-grid]").replaceChildren(
    ...getData().assessments.map((a) => {
      ...
      card.querySelector('[data-value="restingHR"]').textContent =
        a.restingHR || "—";
...
function renderStudentOptions() {
  ...
  document
    .querySelectorAll("[data-student-options],[data-progress-student]")
    .forEach((select) => {
...
function renderStats() {
  ...
  const pendingAssessments = active.filter(
    (student) =>
      !assessedRecently.has(student.id) && !assessedRecently.has(student.name),
  ).length;
  const fatChanges = active
    .map((student) => {
      const entries = d.assessments
        .filter(
          (item) =>
            item.studentId === student.id || item.student === student.name,
        )
        .sort(
          (a, b) =>
            (assessmentDate(a)?.getTime() || 0) -
            (assessmentDate(b)?.getTime() || 0),
        );
      if (entries.length < 2) return null;
      const first = numberFrom(entries[0].bodyFatPercent ?? entries[0].fat);
      const last = numberFrom(
        entries.at(-1).bodyFatPercent ?? entries.at(-1).fat,
      );
  ...
  document.querySelector("[data-stat-workouts]").textContent =
    publishedWorkouts.length;
  ...
  document.querySelector("[data-stat-assessments]").textContent =
    pendingAssessments;
  document.querySelector("[data-stat-assessments-detail]").textContent =
    pendingAssessments
  ...
  document.querySelector("[data-stat-evolution-detail]").textContent =
    averageEvolution === null
      ? "Aguardando reavaliações"
      : "Melhora média na gordura corporal";
  document.querySelector("[data-student-count]").textContent =
    d.students.length;
}
...
function renderDashboardChart() {
  ...
  months.replaceChildren(
    ...series.map((item) => elementSpan(monthLabel(item.date))),
  );
...
function renderSchedule() {
  ...
  const today = allUpcoming.filter((item) =>
  ...
  if (!appointments.length) {
    ...
    empty.textContent =
      "Use “Novo atendimento” para organizar sua agenda presencial.";
...
function renderProgress() {
  ...
  const student =
    getData().students.find((item) => item.name === selected) ||
    getData().students[0];
  if (!student) return;
  ...
  document.querySelector("[data-progress-avatar]").textContent = initials(
    student.name,
  );
  ...
  const averageProgress = workouts.length
    ? workouts.reduce((total, item) => total + Number(item.progress || 0), 0) /
      workouts.length
    : null;
  const start =
    assessmentDate(assessments[0]) ||
    (student.createdAt ? new Date(student.createdAt) : null);
  document.querySelector("[data-progress-start]").textContent = start
    ? shortDate(start)
    : "—";
  ...
  document
    ...
    .replaceChildren(
      ...entries.map((item) => elementSpan(monthLabel(item.date))),
    );
  ...
  updateDifference(
    "weight",
    numberFrom(first?.weightKg),
    numberFrom(last?.weightKg),
    " kg",
  );
  updateDifference(
    "fat",
    numberFrom(first?.bodyFatPercent),
    numberFrom(last?.bodyFatPercent),
    "%",
  );
  updateDifference(
    "waist",
    numberFrom(first?.waistCm),
    numberFrom(last?.waistCm),
    " cm",
  );
  setKpi(
    "performance",
...
export function initDashboard() {
  ...
  document
    .querySelector("[data-exercise-filter]")
  ...
  document
    .querySelector("[data-progress-metric]")
    .addEventListener("change", renderProgress);
  document
    .querySelector("[data-students-table]")
    .addEventListener("click", async (event) => {
      ...
      if (b.dataset.action === "edit")
        window.dispatchEvent(
          new CustomEvent("frs:edit-student", { detail: id }),
        );
      if (b.dataset.action === "access")
        window.dispatchEvent(
          new CustomEvent("frs:manage-access", { detail: id }),
        );
  ...
  document
    .querySelector("[data-workouts-grid]")
    .addEventListener("click", (event) => {
      ...
      if (b.dataset.action === "edit")
        window.dispatchEvent(
          new CustomEvent("frs:edit-workout", { detail: id }),
        );
      ...
      if (
        ...
        window.confirm("Excluir esta ficha de treino?")
      ) {
        updateData((d) => {
  ...
  document
    .querySelector("[data-exercises-list]")
    .addEventListener("click", (event) => {
  ...
  document
    .querySelector("[data-progress-student]")
  ...
  document
    .querySelector("[data-schedule-list]")
    .addEventListener("click", (event) => {
      ...
      if (button.dataset.appointmentAction === "edit")
        window.dispatchEvent(
          new CustomEvent("frs:edit-appointment", { detail: id }),
        );
      if (
        ...
        void removeRecord("appointments", id).catch((error) =>
          showToast(error.message),
        );
